/*  ContentFlowAddOn_DEFAULT, version 0.7.2 
...
new ContentFlowAddOn ('DEFAULT', {
	...
	ContentFlowConf: {
         ...
         */
        calcStepWidth: function(diff, absDiff, numberOfItems, visibleItems) {
            var vI = visibleItems;
            if (absDiff > vI) {
                if (diff > 0) {
                ...
                var stepwidth = diff / numberOfItems;
            } else {
                var stepwidth = diff * ( vI / numberOfItems);
            }
            return stepwidth;
        },
         ...
         * side: -1, 0, 1 :: Position(item)/Math.abs(Position(item)) or 0 
         */
        calcCoordinates: function ( relativePosition, relativePositionNormed, side, size) {
            var rP = relativePosition;
            var rPN = relativePositionNormed;
            var vI = rPN != 0 ? rP/rPN : 0 ; // visible Items

            var f = 1 - 1/Math.exp( Math.abs(rP)*0.75);
            var x =  side * vI/(vI+1)* f; 
            var y = 1;
         ...
         */
        calcRelativeItemPosition: function ( relativePosition, relativePositionNormed, side, size) {
            var x = 0;
            var y = -1;
            return {x: x, y: y};
        },
         ...
         */
        calcZIndex: function ( relativePosition, relativePositionNormed, side, size) {
            return -Math.abs(relativePositionNormed);
        },

        /*
         * calculates and returns the relative font-size of an item
         */
        calcFontSize: function ( relativePosition, relativePositionNormed, side, size) {
            return size;
        },
         ...
         */
        calcOpacity: function ( relativePosition, relativePositionNormed, side, size, endOpacity) {
            return Math.max(1 - ((1 - endOpacity ) * Math.sqrt(Math.abs(relativePositionNormed))), endOpacity);
        }
